...
class RawXDF(BaseRaw):
    ...
    def __init__(self, fname, stream_ids, marker_ids=None, prefix_markers=False, fs_new=None, fs_round=False):
        ...
        if len(stream_ids) > 1 and fs_new is None:
            raise ValueError("Argument `fs_new` is required when reading multiple streams.")
        ...
        if all(_is_markerstream(streams[stream_id]) for stream_id in stream_ids):
            raise RuntimeError(
                "Loading only marker streams is not supported, at least one stream must be a regular stream."
            )
...
def _resample_streams(streams, stream_ids, fs_new):
    ...
    for stream_id in stream_ids:
        ...
        row_start = int(np.floor((streams[stream_id]["time_stamps"][0] - first_time) * fs_new))
...
def read_raw_xdf(
    ...
    **kwargs,
    ...
    return RawXDF(fname, stream_ids, marker_ids, prefix_markers, fs_new, fs_round)
    ...
    with open_xdf(fname) as f:
        ...
        while True:
            ...
            if tag == 1:
                ...
                chunk["content"] = "0x43 0xA5 0x46 0xDC 0xCB 0xF5 0x41 0x0F 0xB3 0x0E 0xD5 0x46 0x73 0x83 0xCB 0xE4"
